...
def create_attention_mask_for_mmu(sequence, eoi_id=128258, return_inverse_mask=True):
    ...
    if return_inverse_mask:
        ...
        return causal_mask
# def create_attention_mask_for_mmu(
#     sequence, 
#     eoi_id=128258, 
#     return_inverse_mask=True,
#     dtype=torch.float32  # 新增：允许指定掩码的目标 dtype（如 bfloat16/float32）
# ):
#     N, L = sequence.shape
#     # 1. 生成因果掩码（bool 类型，不涉及 dtype 问题）
#     causal_mask = torch.tril(torch.ones((N, 1, L, L), dtype=torch.bool)).to(sequence.device)
#     eoi_image = torch.where(sequence == eoi_id)[1]
#     causal_mask[:, :, :, :eoi_image[0] + 1] = 1  # 保留 EOI 前的注意力范围

#     if return_inverse_mask:
#         # 2. 关键修改：用指定的 dtype（如 bfloat16）转换，而非 sequence.dtype（int64）
#         inverted_mask = 1.0 - causal_mask.type(dtype)
#         # 3. 关键修改：用 finfo（浮点类型信息）填充最小值，而非 iinfo（整数）
#         inverted_mask = inverted_mask.masked_fill(
#             inverted_mask.to(torch.bool),  # 保留原有的掩码逻辑
#             torch.finfo(dtype).min  # 适配浮点类型的最小值（如 bfloat16 的最小值）
#         )
#         return inverted_mask
#     else:
#         # 若返回非反向掩码，也转换为指定 dtype
#         return causal_mask.type(dtype)
